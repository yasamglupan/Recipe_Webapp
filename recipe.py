...
def recipe(id):
    ...
    if recipe is None:
        abort(404)
    print(recipe)
    cur.execute('SELECT name FROM Ingredients WHERE id IN\
        (SELECT ingredient_id FROM Recipe_Ingredients WHERE recipe_id=?)', (id, ))
...
def add_recipes():
    ...
    conn = sqlite3.connect('recipe.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO Review (name, rating, review, recipe_id) \
        VALUES (?, ?, ?, ?)', (name, rating, review, recipe_id))
    conn.commit()
