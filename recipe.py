...
def retrive_query_results(sql_statement, fetch_method="all", values=None):
    """
    This is a function that takes a sql statement, opens a connection, runs the sql query, and
    either commits or fetches the results and returns the results


    :sql_statment: This is the sql query
    :fetch_method: This confirms if to fetch one, fetch all, or fetch no results
    :values: If values are passed execute with the values
    :return: returns the results of the query sql_statment
    """
    conn = sqlite3.connect('recipe.db')
    cur = conn.cursor()
    if values:  # if values are passed execute with the values
        cur.execute(sql_statement, values)  # Pass values as a tuple
    else:
        cur.execute(sql_statement)
    if fetch_method == "one":  # Fetch one
        result = cur.fetchone()
    elif fetch_method == "none":  # if fetch method is none commit
        conn.commit()
...
def contact():
    if request.method == 'POST':
        # Sets all the variables to the information inputed
        name = request.form['name']
        email = request.form['email']
        message = request.form['message']
        # SQL staement to insert the contact information into the database
...
def recipe(id):
    ...
    """
    This function passes 3 sql quaries with a recipe id and returns all the
    data so that it can be displayed on the recipe page

    :id: This is the id of the recipe that we are fetching data for
    :return: Returns the general recipe information 
             The ingredients with the quantity
             Any revews written on the recipe 
    """
    ...
    # Execute an SQL query to retrieve ingredient names and quantityfor the recipe
    ...
    # Execute an SQL query to retrieve reviews for the recipe id
